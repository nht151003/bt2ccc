...
public class MainFrame extends JFrame {
    ...
    private void initComponents() {
        ...
        mainPanel.add(searchBarPanel, BorderLayout.NORTH); // Đặt thanh tìm kiếm ở phía trên cùng
    ...
    private JPanel createSearchBarPanel() {
        ...
        gbc.gridx = 0;
        gbc.gridy = 0;
        searchBarPanel.add(new JLabel("Từ khóa:"), gbc);
        ...
        searchButton.addActionListener(e -> {
            ...
            if (selectedTab == 0) {
                // Tìm kiếm trong tab "Tài liệu"
                searchDocumentsByKeyword(keyword);
            } else if (selectedTab == 1) {
                // Tìm kiếm trong tab "Lưu trữ"
                searchArchivedDocuments(keyword);
            }
        });

        return searchBarPanel;
    }
    private void searchDocumentsByKeyword(String keyword) {
        if (keyword.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Vui lòng nhập từ khóa tìm kiếm!", "Thông báo", JOptionPane.WARNING_MESSAGE);
            return;
        }

        List<Document> results = documentDAO.searchDocuments(keyword, null, null, currentUser.getId());
    ...
    private JPanel createDocumentPanel() {
        ...
        // Bảng dữ liệu
        String[] columns = {"ID", "Tiêu đề", "Tác giả", "Lĩnh vực", "Ghi chú", "Từ khóa", "Nội dung/Link", "Loại", "Chủ đề"};
        ...
        try {
            if (documentDAO.moveToArchive(docId, currentUser.getId())) {
                SwingUtilities.invokeLater(() -> {
                    loadDocuments();                    // cập nhật tab Tài liệu
                    tabbedPane.setSelectedIndex(1);    // chuyển sang tab Lưu trữ
                    loadArchivedDocuments();           // cập nhật tab Lưu trữ
                    JOptionPane.showMessageDialog(this,
                ...
                JOptionPane.showMessageDialog(this, "Không thể lưu trữ tài liệu!", "Lỗi", JOptionPane.ERROR_MESSAGE);
            }
        } catch (SQLException ex) {
            ex.printStackTrace();
            JOptionPane.showMessageDialog(this,
    ...
    if (confirm == JOptionPane.YES_OPTION) {
        try {
            if (documentDAO.restoreFromArchive(archiveId, currentUser.getId())) {
                SwingUtilities.invokeLater(() -> {
                    loadArchivedDocuments();          // cập nhật tab Lưu trữ
                    tabbedPane.setSelectedIndex(0);   // chuyển về tab Tài liệu
                    loadDocuments();                  // cập nhật tab Tài liệu
                    JOptionPane.showMessageDialog(this,
